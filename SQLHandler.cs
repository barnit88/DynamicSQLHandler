...
namespace DynamixSQLHandler
{
    public partial class SQLHandler
    {
        ...
        /// <param name="OutPutParamColl">Output Key Value collection for parameters.</param>
        /// <returns>List Key Value collection</returns>
        public List<KeyValuePair<string, object>> ExecuteNonQueryWithMultipleOutputStringObject(string stroredProcedureName, List<KeyValuePair<string, object>> InputParamColl, List<KeyValuePair<string, object>> OutPutParamColl)
        {
            SqlConnection SQLConn = new SqlConnection(ConnectionString);
            try
            {
                ...
                for (int i = 0; i < OutPutParamColl.Count; i++)
                {
                    lstRetValues.Add(new KeyValuePair<string, object>(i.ToString(), SQLCmd.Parameters[InputParamColl.Count + i].Value.ToString()));
                }
                return lstRetValues;
            }
            catch (Exception e)
